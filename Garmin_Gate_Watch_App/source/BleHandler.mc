...
class BleHandler extends Ble.BleDelegate {
    ...
    function initialize() {
        ...
        _connectionWindowTimer = new Timer.Timer();
    ...
    function startConnectionWindow() {
        if (!hasConfiguredPsk()) {
            ...
            _state = STATE_ERROR;
            _statusText = "Set PSK in Connect IQ";
            WatchUi.requestUpdate();
            return;
        }
        resetConnectionWindow();
        startScan();
    }
    ...
    private function hasConfiguredPsk() {
        var psk = Application.Properties.getValue("psk");
        return psk instanceof Lang.String &&
               (psk as Lang.String).length() > 0 &&
               !(psk as Lang.String).equals("SET_IN_CONNECT_IQ");
    }
    ...
    function forceUnpair() {
        ...
        // _device is null while the UI says "Scanning", but Garmin may still
        // retain a disconnected device in the app's paired-device list. Clear
        // every paired device exposing our registered service so switching
        // from the car ESP to the gate ESP works without reinstalling the app.
        var unpairedCount = 0;
        try {
            var devicesToUnpair = [];
            var pairedDevices = Ble.getPairedDevices();
            var pairedDevice = pairedDevices.next();
            while (pairedDevice != null) {
                if (pairedDevice instanceof Ble.Device) {
                    var candidate = pairedDevice as Ble.Device;
                    if (candidate.getService(CarKeyProfile.SERVICE_UUID) != null) {
                        devicesToUnpair.add(candidate);
                    }
                }
                pairedDevice = pairedDevices.next();
            }

            // Finish consuming the iterator before changing the paired-device
            // collection; some watch firmwares invalidate a live iterator.
            for (var i = 0; i < devicesToUnpair.size(); i++) {
                Ble.unpairDevice(devicesToUnpair[i]);
                unpairedCount++;
            }
        } catch (e) {
            System.println("Paired device cleanup error: " + e.getErrorMessage());
        }

        if (_device != null) {
            // It may already have been removed by the paired-device loop.
            try {
                Ble.unpairDevice(_device);
            } catch (e) {
                System.println("Current device unpair error: " + e.getErrorMessage());
            }
            _device = null;
        }
        _state = STATE_IDLE;
        _statusText = "Cleared " + unpairedCount + ", scanning...";
        WatchUi.requestUpdate();
    ...
    // Stop BLE connection attempts after three minutes. Pressing SELECT
    // starts a fresh window through sendPress().
    function onConnectionWindowExpired() as Void {
        if (_device != null && _device.isConnected()) {
            return;
        }
        ...
        if (_device != null) {
            try {
                ...
                System.println("Connection window unpair error: " + e.getErrorMessage());
            }
            _device = null;
        }

        _state = STATE_IDLE;
        _statusText = "Press to reconnect";
        System.println("Connection window expired; scanning stopped");
        WatchUi.requestUpdate();
    }
    ...
    private function resetConnectionWindow() as Void {
        _shouldAutoReconnect = true;
        _connectionWindowTimer.stop();
        _connectionWindowTimer.start(method(:onConnectionWindowExpired), 180000, false);
    }
    ...
    function onScanStateChanged(scanState, status) {
        if (scanState == Ble.SCAN_STATE_OFF && _state == STATE_SCANNING) {
            ...
            if (_device == null) {
                _state = STATE_IDLE;
                _statusText = _shouldAutoReconnect ? "Scanning..." : "Press to reconnect";
                WatchUi.requestUpdate();
    ...
    function onScanResults(scanResults as Ble.Iterator) as Void {
        ...
        if (lastSeenName != null && _state == STATE_SCANNING) {
            // Keep scan diagnostics in the debug log; the watch face should
            // show a stable, readable user-facing state on a round screen.
            System.println("Scan results=" + _scanResultCount +
                ", last=" + lastSeenName + ", RSSI=" + lastSeenRssi);
    ...
    function onConnectedStateChanged(device, state) {
        if (state == Ble.CONNECTION_STATE_CONNECTED) {
            ...
            // expired or the app was stopped.
            if (!_shouldAutoReconnect) {
                try {
                    Ble.unpairDevice(device);
                } catch (e) {
                    System.println("Late connection unpair error: " + e.getErrorMessage());
                }
                return;
            }
    ...
    private function vibrateConnected() {
        vibratePattern([new Toybox.Attention.VibeProfile(100, 80)]);
    }
    ...
    private function vibrateSuccess() {
        vibratePattern([new Toybox.Attention.VibeProfile(100, 200)]);
    }
    ...
    private function vibratePattern(pattern) {
